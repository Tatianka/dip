...
\begin{dosledok}
Trieda regexov s konštatnou hĺbkou vnorenia lookaroundov patrí do $DSPACE(n\log^2 n)$.
\end{dosledok}
\begin{proof}
Podľa lemy \ref{lema_hlbka_lookaroundov} je počet konfigurácií v akceptačnom výpočte najviac $O((rw)^O(1))$ -- existujú konštatny $c_1,c_2\in\N$ také, že $c_1(rw)^{c_2}$ je horné ohraničnie tejto funkcie. Potom hĺbka rekurzie funkcie $TESTUJ$ je $\log(c_1(rw)^{c_2}) = \log c_1 + c_2\log r + c_2\log w = O(\log r+\log w)$, čo je ten istý odhad ako v dôkaze vety \ref{dspace_nlog2n}.
\end{proof}

Teraz nasleduje sľúbený dôkaz vety \ref{dspace_log2n}.
\begin{proof}
Konštrukcia Turingovho stroja bude taká, ako v dôkaze vety \ref{dspace_nlog2n}, s malými zmenami. Budeme označovať $r$ ako dĺžku regexu a $w=n$ dĺžku vstupného slova.

Keďže regex poznáme dopredu, vieme si vyrobiť špeciálny znak na zápis všetkých možných polôh ukazovateľa v regexe ($(r+1)$ nových znakov). Takisto vieme počet spätných referencií, lookaheadov a lookbehindov, preto informácie k nim prislúchajúce si vieme zapísať nad seba do niekoľkých stôp na páske. K týmto stopám pridáme ešte 2 ďalšie -- pre ukazovateľ v slove a ukazovateľ pre spätné referencie. Turingov stroj si bude strážiť, aby zápis adries začínal vždy na tom istom políčku. Potom zápis jednej konfigurácie zaberie $\log w$ políčok.
