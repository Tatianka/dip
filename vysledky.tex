...
$\Gamma$ je pracovná abeceda obsahujúca poschodové symboly s konečným počtom poschodí a slovo w obsahuje v najspodnejšom poschodí vstupné slovo pre regex $\alpha$.
\end{df}

\begin{df}
Konfiguráciu $(\lceil r_1 \dots r_n,\lceil w_1\dots w_m)$ nazývame \textbf{počiatočnou} konfiguráciou a $(r_1 \dots r_n \lceil, w_1\dots w_m \lceil)$ nazývame \textbf{akceptačnou}.
\end{df}
...
Je zakázané odkazovať sa na operácie formy $(?\dots )$, preto ich ani nechceme a nebudeme brať do úvahy v poradí zátvoriek. Z týchto operácií sa v našom modeli nachádza iba pozitívny a negatívny lookaround. Je povolené odkazovať sa na zátvorky vnútri lookaroundu, takže sa vieme odvolať na podslovo, čo sa zhoduje s pozitívnou formou (ak lookaround považujeme za neindexovateľné zátvorky, stačí ho prepísať do formy $\lookahead (\dots ))$ a vieme sa referencovať na jeho obsah). Problém nastáva pri jeho negatívnej verzii -- podľa definície nesmie nájsť žiadnu zhodu, inak sa výpočet zastaví. Preto po akceptácii nedefinuje žiadne podslovo, na ktoré by sme sa mohli odvolať. To isté platí o ľubovoľných zátvorkách v jeho vnútri.

Avšak počas výpočtu negatívny lookaround spätné referencie a indexovateľnosť zátvoriek využívať môže. Preto definícia bude závisieť od polohy ukazovateľa v regexe -- bude používať indukciu vzhľadom na počet negatívnych lookaroundov, v ktorých je vnorený.
...
Nech je ukazovateľ vnorený v 1 negatívnom lookarounde. Potom najprv posúdime vonkajší regex podľa  predošlého kritéria a potom rovnako posudzujeme každú zátvorku ( v regexe vnútri tohto negatívneho look\-aroundu, tak akokeby to bol hlavný regex.

Predpokladajme, že vieme posúdiť indexovateľnosť zátvoriek, pokiaľ je ukazovateľ vnorený v $k-1$ negatívnych lookaroundoch a nech teraz ukazovateľ vnorený v $k$ negatívnych lookaroundoch. Indexovateľnosť zátvoriek posúdime podľa horeuvedeného kritéria akokeby bol najvnútornejší regex hlavný regex. Všetky ostatné zátvorky vo vonkajších negatívnych lookaroundoch máme posúdené.
\end{df}
