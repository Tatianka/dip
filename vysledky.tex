...
\end{lema}
\begin{proof}
Nech $L_1,L_2,L_3 \in \R$. Ukážeme, že $L=L_1(?<!~L_2)L_3 \in \R$. Postup je podobný ako pri negatívnom lookaheade, skonštruujeme $A_2''$ také, že bude akceptovať práve vtedy, keď $(?<~!L_2)$.

Máme $A_2$ pre $L_2$, vytvoríme $A_2'$ podľa dôkazu pre negatívny lookahead, tentokrát však pridávame znaky zo $\Sigma_1$. Pre $A_2''$ potrebujeme trocha inú $\delta$-funkciu:
\begin{eqnarray*}
 \forall q \in K_2'  ~ \forall a \in \Sigma_2' &:&  \delta_2''(q,a)=\delta_2'(q,a) \\
 \forall q \in K_2' \setminus F_2'  ~ \forall a \in \Sigma_2' &:&  \delta_2''(q,\varepsilon)=q_A \\
 \forall a \in \Sigma_2' &:& \delta_2''(q_A,a)=q_Z 
...
Tvrdíme $ L_1(?<=L(A_2''))L_3 =L_1(?<!~L_2)L_3= L $. Opäť vynecháme výpočty $A_1$ a $A_3$.

$\subseteq :$ Majme akceptačný výpočet $A_2''$ na podslove $w$ slova $uwv$. 

$\supseteq :$

Celý mechanizmus lookbehindu zabezpečuje automat pre $L$ - t.j. nedeterministicky si tipne, kde je začiatok slova a akceptačný stav skontroluje až tam, kde končí lookbehind (ak tam nie je, automat sa zasekne, inak výpočet pokračuje). Keďže výpočet $A_1$ a $A_2$ prebieha naraz, pracujeme s karteziánskym súčinom stavov. 
\end{proof}
